...
def insertString(a,b,c,d,e,f,g,h,i,j,k,l,m,n):    
    insert_count="insert into live_info (channel_guid,url_bitrate,url_format,v_codec,v_aspect_ratio,v_frame_rate,v_stream_id,v_resolution,a_codec,a_bitrate,a_sound_track,a_sampling_rate,a_stream_id,channel_type) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (a,b,c,d,e,f,g,h,i,j,k,l,m,n)
    print insert_count
    return insert_count
...
def readLogFile():
    ...
    for line in lines:
        ...
        if url_bitrate=='1400k':    
            ...
            channel_type=1
    insert_str=insertString(channel_guid,url_bitrate,url_format,v_codec,v_aspect_ratio,v_frame_rate,v_stream_id,v_resolution,a_codec,a_bitrate,a_sound_track,a_sampling_rate,a_stream_id,channel_type)
    return insert_str
...
def insertLiveInfoDB():
    ...
    if live_file_list is not '':
        try:
            conn=MySQLdb.connect(host='127.0.0.1',user='root',passwd='db',db='so_ads',port=3306, charset="utf8", cursorclass = MySQLdb.cursors.DictCursor)
            cur=conn.cursor()
