...
def get_snfile(SNFile_PATH):
    try: sndir=os.listdir(SNFile_PATH)
    except OSError,e:
           return None
    else:
...
def ftp_client(fpath,fname,lip):
    ...
    try: 
        ...
        ftp.login('loguser','ZWSZFxdKAs')
         ...
         print e.message
         print 'Plese check FTP Server configuration info'
        ...
        print ftp.storbinary(fcmd,file_handler,1024)
...
def usage(args):
    print '''%s -m (normal|norotate|onlyupload) -f SN_file
...
def main():
    try:  opts,args=getopt.getopt(sys.argv[1:],"hm:f:",['help'])
    ...
    for loginfo in log_conf:
        ...
        if os.path.isdir(log_dir):
            ...
            smail(toaddrs,'log dir %s is not exist,check SN_file configuration' %log_dir,'%s Log Upload Error Notify' %IP)
            continue
