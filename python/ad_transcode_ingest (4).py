...
class Ad_Transcode_Ingest(object):
    ...
    def get_init_ad_list(self, ad_provider):
        ...
        try:
            ...
            sql = "select ad_id,ad_hash,ad_downloadurl from ad_info where ad_prostatus='%d' and ad_provider='%s';" % (
            ad_pro_status['init'], ad_provider)
            cur.execute(sql)
            conn.commit()
            data = cur.fetchall()
            if 0 == len(data):
    ...
    def get_downloaded_ad_list(self, ad_provider):
        ...
        try:
            ...
            cur = open_db_cur(conn)
            sql = "select ad_id, ad_hash, ad_downloadurl from ad_info where ad_prostatus='%d' and ad_provider='%s'" % (
            ad_pro_status['downloaded'], ad_provider)
            cur.execute(sql)
            conn.commit()
            data = cur.fetchall()
            if 0 == len(data):
    ...
    def transcode_ad(self, ad, live_type):
        ...
        transcode_cmd = ('%s -y -i %s -vcodec libx264 -streamid 1:257 -streamid 0:256 -s %s '
                         ...
                         '-profile:a aac_low -ar %s -b:a %s -vol 80 '
                         # '-vf movie=/opt/logo/logo.png[logo]\;[in][logo]overlay=main_w-overlay_w-100:0[out]'
                         '-vf movie=/opt/logo/logo.png[logo]\;[in][logo]overlay=10:10[out] %s'
                         % (
    ...
    def get_transcoded_ad_list(self, ad_provider):
        ...
        try:
            conn = open_db_conn(server_ip, 'root', 'db', 'so_ads')
            cur = open_db_cur(conn)
            sql = "select ad_id, ad_hash from ad_info where ad_prostatus='%d' and ad_provider='%s'" % (
            ad_pro_status['transcoded'], ad_provider)
            cur.execute(sql)
            conn.commit()
            data = cur.fetchall()
            if 0 == len(data):
    ...
    def get_imcomplete_ad_list(self, ad_provider):
        ...
        try:
            conn = open_db_conn(server_ip, 'root', 'db', 'so_ads')
            cur = open_db_cur(conn)
            sql = "select * from ad_info where ad_prostatus = 0 or ad_prostatus = 1 or ad_prostatus = 2 and ad_provider='%s'" % (
                ad_provider)
            cur.execute(sql)
            conn.commit()
            data = cur.fetchall()
...
if __name__ == "__main__":
    obj = Ad_Transcode_Ingest('8080')
    # src_file = 'http://172.16.1.115:8080/b405c7d93abffc6d61823a1de47941c1.ts'
    # guid = 'test_vod'
    # print obj.ingest_xml(src_file,guid)
    ad = {'ad_hash': u'b405c7d93abffc6d61823a1de47941c1',
          'ad_downloadurl': u'http://172.16.1.115:8080/ad/logoguazi15.ts', 'ad_id': 36L}
    live_type = {'audiorate': '42k', 'reselution': '1280x720', 'bitrate': '1000k', 'name': 'type0', 'aspect': '16:9',
                 'framerate': '25.00', 'audiosample': '44100', 'type': 0}
    obj.transcode_ad(ad, live_type)
