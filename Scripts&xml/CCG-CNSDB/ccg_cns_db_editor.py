...
def get(guid):
    ...
    if mids is None:
        print
        "Didn't find any record match guid", guid
        return
    print
    "Those machined have content", guid
    for mid in mids:
        print
        "\t", mid
...
def set(guid, mids):
    ...
    for mid in mids:
        if not re.match("[0-9a-fA-F]{32}", mid):
            print
            "Mid %s is not correct, it must be 32 bytes hex string.", mid
            return
    db.update(guid, mids)
    print
    "Set successed."
...
def usage():
    print
    "Usage: %s {get|set} guid [mid] ... " % sys.argv[0]
    sys.exit(1)
