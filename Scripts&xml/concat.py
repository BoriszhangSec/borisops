...
os.system(
    "ffmpeg -i concat:'" + video_path_list[0] + "|" + video_path_list[1] + "' -vcodec copy -acodec copy -f mpegts tmp1")
for i in range(2, len(video_path_list)):
    os.system("ffmpeg -i concat:'tmp" + str(i - 1) + "|" + video_path_list[
        i] + "' -vcodec copy -acodec copy -f mpegts tmp" + str(i))
    time.sleep(2)
